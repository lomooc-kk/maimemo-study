...
退出码非 0 就改到通过为止；它还会报单词表里有几条缺音标。中文释义和音标接口都不提供，都得自己写，条目写成 `单词 /英式音标/ 释义`，释义控制在 12 个汉字内。成品交 Word，只放正文和单词表两部分；`md2docx.py` 会把「单词表」小节自动排成两列 9 磅小字（音标 8.5 磅灰字、每条不跨页），打印省纸，所以条目要短。文件名带上日期区间（如 `错词短文-9月11-12日.docx`）。
