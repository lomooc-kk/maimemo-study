...
退出码非 0 就改到通过为止；它还会报单词表里有几条缺音标。中文释义和音标接口都不提供，都得自己写，条目写成 `单词 /英式音标/ 释义`（释义一般 20 字内，放不下会自动换行）。成品交 Word，只放正文和单词表两部分；`md2docx.py` 会把「单词表」小节自动排成两列——单词 9.5 磅加粗、音标 8.5 磅灰字、中文释义 8 磅，每条不跨页。文件名带上日期区间（如 `错词短文-9月11-12日.docx`）。
