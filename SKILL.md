...
- 日期按**北京时间**算。接口返回的是 UTC 时间戳（北京时间某天学的词，返回的时间戳落在前一天 16:00Z），脚本已统一换算，不要自己截前 10 位。
...
- **一条故事线**：主角 + 想要的东西 + 阻力与转折 + 结尾回响，四样都要有；每换一节都能回答"为什么接着讲这个"。骨架也要轮换（问题→解决、时间线、倒叙、访谈、双线对照、调查、旅程、得失、传承、误会—澄清），别总是时间线；选好后把"骨架 + 主角 + 贯穿物"记进 style-log。写完自检：能用一句话说出"谁想做什么、遇到什么、最后怎样"，且把每段第一句连起来是一条推进线而不是并列清单。
...
退出码非 0 就改到通过为止；它还会报有几条生词缺音标或缺词性。中文释义和音标接口都不提供，都得自己写，条目写成 `单词 词性 /美式音标/ 释义`，词性（`n.` `v.` `adj.` 等）必写、常用词给 1–3 个义项，例如 `harsh adj. /hɑːrʃ/ 严厉的；恶劣的`。音标一律用美式（General American）：卷舌音要写出来（car `/kɑːr/`、hard `/hɑːrd/`），go 写 `/ɡoʊ/`，law 写 `/lɔː/`，不要用英式的 ɒ、əʊ 或不写 r。
