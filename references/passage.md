...
2. **每段后面紧跟该段的中文译文和该段用到的生词**，顺序是：英文段落 → 中文译文 → 生词清单。生词按在段内出现的先后排，条目写成 `单词 词性 /英式音标/ 释义`，例如 `cottage n. /ˈkɒtɪdʒ/ 小屋；村舍`、`plumb v./n. /plʌm/ 探测深度；垂直的；铅锤`。这样读一段就能立刻对照译文、顺手过一遍词，比把词表堆在文末好用。

   **词性必写**：`n.` `v.` `adj.` `adv.` `prep.` `conj.` `pron.` `num.` `int.`，一个词常用几种就都写上，写成 `n./v.`。**释义给常用的 1–3 个**，按常用程度排，义项之间用 `；` 隔开；常见的词别只给一个释义。

   **音标必须写对**，这是硬要求：一律用英式 IPA，带主重音 `ˈ`；多音节词不能漏重音，也不能标两个。拿不准的词查一下再写：
