...
- 成品**只交 PDF**，内容是「每段英文 + 该段中文译文 + 该段生词」交替往下排。不要加导语、使用说明、数据来源说明、落款；用户明确说不要翻译时才省掉译文。
- 做法：先写一份 Markdown 中间稿放在本次对话目录的 `work/`，再一条命令转成 PDF：

  ```bash
  powershell -ExecutionPolicy Bypass -File scripts/md2pdf.ps1 -Md work/src.md `
    -Pdf "outputs/错词短文-9月11-12日-中等.pdf" -Title "标题"
  ```

  它内部先生成一份临时 `.docx`（放在 `work/`，转完自动删除），再用本机 Microsoft Word 导出 PDF，排版和 Word 里完全一致。**Word 文件不是交付物**，不要放进 `outputs/`；用户需要留档时加 `-KeepDocx`。没装 Word 时会直接报错，这时跟用户说明，不要擅自把 Word 当成品交出去。转换完可以抽查一下 PDF 能打开、页数正常。
- 中间稿的写法：文章标题写在命令行参数里（`-Title`），正文用 `##` 做小节标题；英文段落直接写，中文译文单独一段（脚本按"含汉字、几乎不含英文"自动认出来，排成 9 磅小字），该段的生词用 `- 单词 词性 /音标/ 释义` 一串列表写在译文后面（脚本自动排成两列小字）。`md2docx.py` 还支持 `**加粗**`、表格、引用和代码块。
- 需要 python-docx。`py -3 -m pip install python-docx` 装上即可；已经装好 python-docx 的解释器直接用。
- 用户自用材料，不需要渲染和逐页排版检查；生成后确认 PDF 能打开、页数正常、内容齐全即可。
- 中间稿和临时文件放 `work/`，最终 PDF 放到该对话目录的 `outputs/`。用户指定了别的路径就以用户说的为准。
- 文件名带上日期区间和难度档，如 `错词短文-9月11-12日-简单.pdf`，回头好找。文章标题用英文，中文只出现在译文和生词释义里。
