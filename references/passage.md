...
2. **每段后面紧跟该段的中文译文和该段用到的生词**，顺序是：英文段落 → 中文译文 → 生词清单。生词按在段内出现的先后排，条目写成 `单词 词性 /美式音标/ 释义 ｜ 记忆钩子`，例如 `harsh adj. /hɑːrʃ/ 严厉的；恶劣的；刺耳的 ｜ harsh winter 严冬；harshly 严厉地`、`proximity n. /prɑːkˈsɪməti/ 接近；邻近 ｜ 词根 proxim（近）+ -ity`。这样读一段就能立刻对照译文、顺手过一遍词，比把词表堆在文末好用。

   **词性必写**：`n.` `v.` `adj.` `adv.` `prep.` `conj.` `pron.` `num.` `int.`，一个词常用几种就都写上，写成 `n./v.`。**释义给常用的 1–3 个**，按常用程度排，义项之间用 `；` 隔开；常见的词别只给一个释义。

   **每条都要有记忆钩子**，用 `｜` 和释义隔开，6–20 字，四选一：常用搭配（`harsh winter`）、词根词缀（`proxim 近 + -ity`）、形近对比（`distinguish / extinguish`）、一句联想或谐音。只有释义、没有钩子的条目最容易被忘掉；`check_passage.py` 会报缺钩子的条目。
...
- 中间稿的写法：文章标题写在命令行参数里，正文用 `##` 做小节标题；英文段落直接写，中文译文单独一段（脚本按"含汉字、几乎不含英文"自动认出来，排成 9 磅小字），该段的生词用 `- 单词 词性 /音标/ 释义 ｜ 钩子` 一串列表写在译文后面（脚本自动排成两列小字，钩子排在条目末尾的灰色小字）。`md2docx.py` 还支持 `**加粗**`、表格、引用和代码块。
