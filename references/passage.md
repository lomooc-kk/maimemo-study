...
2. **单词表跟在文章最后**，按词在文中出现的顺序排列。条目写原形；正文里用的是变形时用括号注出，如 `weep 哭泣；流泪（文中作 wept）`。每个词一到两个常用义项，条目简短，一般不超过 12 个汉字，例如 `cottage 小屋；村舍`、`plumb 垂直的；探测深度`。中文释义接口拿不到，要自己写；不要搬词典的长解释，也不要用只在本文成立的临时义。
